    ...
    async saveAllApiCryptos() {
        try {
            ...
            await Promise.all(data.map(async coin => {
                const existingCoin: ApiCoinEntity = await this.apiCoinEntityRepository.findOne({
                    where: {
                        symbol: coin.symbol,
                    }
                });
                ...
                if (!existingCoin) {
                    ...
                    console.warn(`La cryptomonnaie avec le symbole ${coin.symbol} existe déjà dans la base de données.`);
                }
            }));
