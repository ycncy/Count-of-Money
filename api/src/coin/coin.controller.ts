...
export class CoinController {
    ...
    @ApiOperation({ summary: 'Get daily information about one or multiple cryptocurrencies' })
    @ApiResponse({ status: 200, type: [ListCoinInfoModel], description: 'Successfully retrieved cryptocurrency information.' })
    ...
    @ApiResponse({ status: 200, type: CoinInfoModel, description: 'Successfully retrieved cryptocurrency history.' })
    @ApiResponse({ status: 404, description: 'Cryptocurrency not found.' })
    @HttpCode(200)
    @Get(':coinID/history/:period')
    async getHistoryByCoinId(@Param('coinID') coinID: number, @Param('period') period: string) {
    ...
    @ApiResponse({ status: 200, description: 'Successfully retrieved cryptocurrency.' })
    @ApiResponse({ status: 404, description: 'Cryptocurrency not found.' })
    @HttpCode(200)
    @Get(':coinID')
    ...
    @ApiResponse({ status: 201, description: 'Successfully created a new cryptocurrency.' })
    @ApiResponse({ status: 409, description: 'Conflict - Cryptocurrency already exists.' })
    @HttpCode(201)
    @Post()
    ...
    @ApiResponse({ status: 204, description: 'Successfully deleted cryptocurrency.' })
    @ApiResponse({ status: 404, description: 'Cryptocurrency not found.' })
    @HttpCode(204)
    @Delete(':coinID')
